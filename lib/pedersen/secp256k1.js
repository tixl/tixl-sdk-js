...
const malloc = memUtils.malloc;
const charStar = memUtils.charStar;
const charStarArray = memUtils.charStarArray;

const Module = self.Module._malloc ? self.Module : self;

/**
 *  @summary Generate a pedersen commitment.
 *  @description Blinding factors can be generated and verified in the same way as secp256k1 private keys for ECDSA.
 *
 ...
 *  @exports
 */
export function commit(blindFactor, value) {
  if (!blindFactor || blindFactor.length !== 32) throw new TypeError('blindFactor is a required 32 byte array');
 ...
 *  @exports
 */
export function blindSum(blinds, nneg = blinds.length) {
  const sum = malloc(32);
  const ret = Module.ccall(
 ...
 *
 */
export function verifySum(commits, negativeCommits, excess = 0) {
 ...
 *
 */
export function rangeProofSign(minValue = 0, commitment, commitBlind, nonce, base10Exp = 0, minBits = 0, actualValue) {
 ...
 *  @arg {Array} proof
 */
export function rangeGetInfo(proof) {
  const exp = charStar(4);
  const mantissa = charStar(4);
  const min = charStar(8);
  const max = charStar(8);
  const secp256k1_rangeproof_info = Module.cwrap('secp256k1_rangeproof_info', 'number', [
    'number',
    'number',
    'number',
    'number',
  ...
  const ret = secp256k1_rangeproof_info(ctx(), exp, mantissa, min, max, charStar(proof), proof.length);
